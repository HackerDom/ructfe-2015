		return word
	...
	def randsmile(self):
		return random.choice([
			'＜(。_。)＞', '≦(._.)≧', '<(￢0￢)>', '<(．＿．)>', '<(*--*)>', '<(＠´＿｀＠)>', '(;¬_¬)', '（；¬＿¬)', '(｡+･`ω･´),',
			'｡゜(｀Д´)゜｡', '(ﾟДﾟ)＜!!', '(‡▼益▼)', '(,,#ﾟДﾟ)', '(҂⌣̀_⌣́)', '(；¬д¬)', '（;≧皿≦）', '(╬ﾟ◥益◤ﾟ)', '(╬⓪益⓪)', '[○･｀Д´･○]',
			'૮(ᵒ̌▱๋ᵒ̌)ა', '(⁎˃ᆺ˂)', '(ꐦ°᷄д°᷅)', '((╬●∀●)', '(╬Ò‸Ó)', '(>д<)', '(*｀益´*)', '(☞◣д◢)', '<(｀^´)>', '(;｀O´)o', '(ꐦಠ皿ಠ)',
			'(*｀Ω´*)', '(╬ಠ益ಠ)', '(ு⁎ு)྆', '(╬⓪益⓪)', '（╬ಠ益ಠ)', '(●o≧д≦)o', '(๑･`▱´･๑)', '(☄ฺ◣д◢)', '(╬◣益◢)', '（╬ಠ益ಠ)',
			'（♯▼皿▼）', '（○｀Ｏ´○）', '(^-人-^)', '(^・ω・^)', '(=;ェ;=)', '(=^-ω-^=)', '(=^･ω･^)y＝', '~(=^‥^)_旦~', 'ヽ(^‥^=ゞ)',
			'b(=^‥^=)o', '=＾●⋏●＾=', '｡＾･ｪ･＾｡', '(^..^)ﾉ', '(・∀・)', '(.=^・ェ・^=)', '((≡^⚲͜^≡))', '(^･o･^)ﾉ”', '(^._.^)ﾉ', '(^人^)',
			'(=；ェ；=)', '(=｀ω´=)', '(=｀ェ´=)', '（=´∇｀=）', '(=^･^=)', '(=^･ｪ･^=)', '(=^‥^=)', '(=ＴェＴ=)', '(=ｘェｘ=)', '＼(=^‥^)/’`',
			'~(=^‥^)/', '└(=^‥^=)┐', 'ヾ(=ﾟ･ﾟ=)ﾉ', 'ヽ(=^･ω･^=)丿', 'd(=^･ω･^=)b', 'o(^・x・^)o', 'V(=^･ω･^=)v', '(⁎˃ᆺ˂)', '(,,^・⋏・^,,)',
			'(ะ^・ω・^ะ)', '(ะ`・ω・´ะ)', 'ฅ(≚ᄌ≚)', '(≚ᄌ≚)ƶƵ', '(≚ᄌ≚)', '(^･ｪ･^)', '(^-x-^*)', 'σ((=ﾟｴﾟ=))', '((≡ﾟ♀ﾟ≡))', '~(=^‥^)ノ☆',
			'(^･ω･^=)~', '┌(=^‥^=)┘', '┌(=^‥^=)┐', '└(=^‥^=)┘', '(^･o･^)ﾉ”', '(=･ｪ･=?', 'ﾍ(^･･^=)~', '(´pゝω･)', 'ﾍ(=^･ω･^=)ﾉ', '=^∇^*=',
			'~(=^–^)', 'v(=∩_∩=)', 'o(=´∇｀=)o', '|n＾ω＾|η', '(o^‥^)o', '(＾*･.･*＾)', '~(=^┬┬^)', 'o(^・x・^)w', 'ω(=＾・＾=)ω', '(^=˃ᆺ˂)',
			'(ृ˄·͈༝·͈˄ृ)', '(ृʾ́꒳ʿ̀ृ)ु', '₍ᵔ·͈༝·͈ᵔ₎', '(˙̂·̫ॅ˙̂)', '˄·͈༝·͈˄₎.｡oO'
		])
	...
	def randuser(self, randlen):
		...
		while True:
			...
			if select < 90:
				...
				login = self.randsmile() + rlogin
			else:
				login = self.randchword(4, 6) + rlogin
			b = login.encode('utf-8')
			if len(b) >= 4 and len(b) <= 24:
				break
	...
	def randbdate(self):
		return self.randomdate('%d.%m.%Y', "01.01.1900", "01.01.2000")
	...
	def get(self, addr, flag_id, flag, vuln):
		...
		if vuln == 1:
			...
			if not str(result).find(flag):
				print('not found joined citizen')
